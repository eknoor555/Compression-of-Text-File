import heapq#this is inbuilt min heap
import os
...
class HuffmanCoding:
    ...
    def compress(self):
        file_name,file_extension=os.path.splitext(self.path)#here file name is sample and split.text separates on the basis of . in this case it is .txt
        output_path=file_name+ ".bin"#in output file we will get .bin
        with open(self.path,'r+') as file, open(output_path,'wb') as output:#here wb specifies write binary in the output file
            text=file.read()#it will help in read the file which is open as file
            text=text.rstrip()#it will remove the spaces from file and give the actual text
...
#path="C:/Users/HP/Downloads/sample.txt"
